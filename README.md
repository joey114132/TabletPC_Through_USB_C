...
현재 이 세팅은 사용 중인 태블릿 웹 브라우저의 1:1 매칭 해상도인 **`1228x768`**로 사전 튜닝되어 있어 배율 스케일링 오버헤드가 없고 반응 속도가 가장 빠릅니다.

만약 더 높은 해상도로 화면을 선명하게 만들고 싶다면 스크립트를 열어 손쉽게 해상도를 바꿀 수 있습니다:
1. `start_tablet_monitor.sh` 파일을 텍스트 에디터로 엽니다.
2. 스크립트 최상단의 `RESOLUTION` 변수값을 편집하고 저장합니다:
   ```bash
   # 예시: 16:10 표준 고해상도로 변경하고 싶을 때
   RESOLUTION="1920x1200"
   ```
3. 모니터를 다시 구동하면 자동으로 변경된 해상도로 RDP 프로필이 갱신되어 가상 화면 크기가 리사이징됩니다.
